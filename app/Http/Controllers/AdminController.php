...
public function SendTestMail(Request $request)
{
    ...
    public function generateUniqueRandomCode($length, $keyspace, $existingCodes) {
        ...
        $maxAttempts = 100000; // Maximum attempts to generate a unique code
        $attempts = 0;
    
        do {
            $randomCode = 'pt' . random_str($length - 1, $keyspace); 
            $attempts++;
    ...
    public function generateUniqueRandomCodeImport($length, $keyspace, $existingCodes) {
        $randomStr = function (
            int $length = 64,
            string $keyspace = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'
        ): string {
        ...
        do {
            $randomCode = 'pt' . $randomStr($length - 1, $keyspace);
            $attempts++;
        } while (in_array($randomCode, $existingCodes) && $attempts < $maxAttempts);
    ...
    public function import_users(Request $request)
{
    ...
    $users = $request->input('users'); // Get the users array from the request

    $duplicateEmailCount = 0; // Counter for duplicate emails
    $duplicateEmails = []; // Array to store duplicate emails

    foreach ($users as $userData) {
        try {
            $existingCodes = User::pluck('activate_code')->toArray();
            $keyspace = 'abcdefghijklmnopqrstuvwxyz1234567890';
            $randomCode = $this->generateUniqueRandomCodeImport(7, $keyspace, $existingCodes);
            $user = User::create([
            ...
            if (!empty($userData['mobile_number'])) {
                ...
                // Split the mobile_number string into an array using common delimiters
            ...
            if ($e->getCode() === '23000') { // Check for duplicate entry (SQLSTATE 23000)
                $duplicateEmailCount++;
