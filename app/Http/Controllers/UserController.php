...

{
...

    {
        ...
        $userinfo = User::select('id', 'name', 'email', 'littlelink_name', 'littlelink_description', 'theme', 'role', 'block', 'activate_status')->where('id', $id)->first();

        $profinfo = ProfessionalInformation::select('user_id', 'title', 'company', 'location', 'country', 'email', 'mobile', 'role')->where('user_id', $id)->first();

        $information = User::select('name', 'littlelink_name', 'littlelink_description', 'theme')->where('id', $id)->get();

        

        /*if ($userinfo->activate_status == 'deactivated' || $userinfo->block == 'yes' && $userinfo->activate_status == 'deactivated' || $userinfo->activate_status == 'activated' && $userinfo->block == 'yes' || $userinfo->activate_status == 'deactivated' && $userinfo->block == 'no') {

            //return abort(404);

            return view('panel.blocked');

        }*/



        if ($userinfo->activate_status == 'deactivated') {
        ...
        /*$foundVCard = $links->contains(function ($link) {

            return $link->name === 'vcard';
        ...
        if ($foundVCard) {
            ...
            // Get the first 'vcard' link

            $vcardLink = $links->first(function ($link) {
            ...
            if ($vcardLink) {
                ...
                $vcardData = json_decode($vcardLink->link, true); // true for associative array
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    ...
                    // Extract specific fields into PHP variables

                    $prefix = $vcardData['prefix'];

                    $first_name = $vcardData['first_name'];

                    $middle_name = $vcardData['middle_name'];

                    $last_name = $vcardData['last_name'];
                    ...
                    $cell_phone = $vcardData['cell_phone'];



                    // Log extracted data for verification or further processing

                    Log::info('Extracted vCard data', [
                    ...
                    Log::error('Failed to decode JSON from vCard link.');
        ...
        return view('linkstack.linkstack', ['userinfo' => $userinfo, 'profinfo' => $profinfo, 'information' => $information, 'links' => $links, 'littlelink_name' => $littlelink_name]);
