    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.E-Mail'), "email_verified_at")
                ...
                ->html(),
            Column::make(__('messages.Status'), "activate_status")
                ->sortable()
                ->format(function ($value, $row, Column $column) {
                    if ($row->role === 'admin' && $row->id === 1) {
                        return '<div class="text-center">-</div>';
                    } else {
                        $route = route('activeStatus', ['activate_status' => $row->activate_status, 'id' => $row->id]);
                        if ($row->activate_status === 'deactivated') {
                            $badge = '<div class="text-center"><a style="cursor:pointer" data-id="' . $route . '" class="user-status text-danger"><span class="badge bg-danger">' . __('messages.Inactive') . '</span></a></div>';
                        } elseif ($row->activate_status === 'activated') {
                            $badge = '<div class="text-center"><a style="cursor:pointer" data-id="' . $route . '" class="user-status text-danger"><span class="badge bg-success">' . __('messages.Active') . '</span></a></div>';
                        }
