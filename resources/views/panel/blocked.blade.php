    ...
    <x-auth-card>
        ...
        <div style="max-width:480px" class="container mt-5 w-100">
          <div class="card p-5">
              ...
              <h2 class="mb-2 text-center mt-4">{{__('messages.Activation Status')}}</h2>
              <p class="text-center"><b>{{__('messages.auth_pending_activation')}}</b></p>
              <p class="text-center mb-4">{{__('messages.auth_desc_activation')}}</p>

                <form method="GET" action="{{ route('activate.profile') }}">
                    @csrf
