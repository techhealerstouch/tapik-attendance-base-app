...
<style>
    ...
    .toggle-section-checkbox .form-check-label {
        font-size: 1.1rem;
        cursor: pointer;
    }
</style>
            ...
            <div class="card professional-card">
                ...
                <div class="card-body p-4">
                    ...
                    <form method="POST" action="{{ route('events.store') }}">
                        ...
                        <!-- Food Services Toggle Checkbox -->
                        <div class="toggle-section-checkbox">
                            <div class="form-check form-switch">
                                <input class="form-check-input" 
                                       type="checkbox" 
                                       role="switch" 
                                       id="enableFoodServices">
                                <label class="form-check-label" for="enableFoodServices">
                                    <i class="bi bi-egg-fried me-2"></i>
                                    <strong>Enable Food Services for this event</strong>
                                </label>
                                <br>
                                <small class="form-text">Check this box if you want to offer food services during the event</small>
                            </div>
                        </div>

                        <!-- Food Services Section -->
                        <div id="foodServicesSection" style="display: none;">
                            <div class="section-header">
                        ...
                        <div class="toggle-section-checkbox">
                            <div class="form-check form-switch">
                                <input class="form-check-input" 
                                       ...
                                       role="switch" 
                                       id="enableTables">
                                <label class="form-check-label" for="enableTables">
                                    <i class="bi bi-table me-2"></i>
                                    <strong>Add Event Tables</strong>
                                </label>
                                <br>
                                <small class="form-text">Check this box if you want to add tables for seating arrangements</small>
                            </div>
                        </div>
                        ...
                        <div id="tablesSection" style="display: none;">
                            <div class="section-header">
                                ...
                                <h5>Event Tables (Optional)</h5>
                            </div>

                            <div class="alert alert-info alert-professional d-flex align-items-center mb-4" role="alert">
                                <i class="bi bi-info-circle me-3 fs-4"></i>
                                <div>
                                    <strong>Note:</strong> You can add tables now or later from the Seat Management page. Tables help organize seating arrangements for your event.
                                </div>
                            </div>

                            <div class="mb-3">
                                <button type="button" class="btn btn-outline-primary" id="addTableRowBtn">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            <p class="small mb-0">Optionally enable food services and set quantities if needed.</p>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        // Toggle Food Services Section
        $('#enableFoodServices').on('change', function() {
            if ($(this).is(':checked')) {
                $('#foodServicesSection').slideDown(300);
            } else {
                $('#foodServicesSection').slideUp(300);
                // Clear selections when hiding
                $('.select-food-services').val(null).trigger('change');
                $('#foodServiceQuantities').hide();
                $('#quantityInputs').empty();
            }
        });

        // Toggle Tables Section
        $('#enableTables').on('change', function() {
            if ($(this).is(':checked')) {
                $('#tablesSection').slideDown(300);
            } else {
                $('#tablesSection').slideUp(300);
                // Clear tables when hiding
        ...
        // Add table row
        $('#addTableRowBtn').on('click', function() {
            tableCounter++;
            const tableRow = `
                <div class="table-input-card mb-3" data-table-index="${tableCounter}">
                    <div class="quantity-card">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h6 class="mb-0">
                                <i class="bi bi-table me-2"></i>Table ${tableCounter}
                            </h6>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" 
                                       name="tables[${tableCounter}][name]" 
                                       class="form-control" 
                                       placeholder="e.g., VIP Table, Table 1"
                                       required>
                                <small class="form-text">Give this table a descriptive name</small>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Number of Chairs *</label>
                                <input type="number" 
                                       name="tables[${tableCounter}][chairs]" 
                                       class="form-control" 
                                       placeholder="e.g., 6"
                                       min="1"
                                       max="50"
                                       required>
                                <small class="form-text">How many people can sit at this table?</small>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-check">
                                    <input class="form-check-input" 
                                           type="checkbox" 
                                           name="tables[${tableCounter}][manual_assignment]" 
                                           value="1" 
                                           id="manual_${tableCounter}">
                                    <label class="form-check-label" for="manual_${tableCounter}">
                                        <i class="bi bi-hand-index text-warning"></i> <strong>Manual Assignment</strong>
                                        <small class="text-muted d-block">
                                            When checked, automatic seat assignment will skip this table during attendance scanning. 
                                            You'll need to manually assign seats through the Seat Management page.
                                        </small>
                                    </label>
                                </div>
                            </div>
            ...
            `;
            $('#tablesContainer').append(tableRow);
        });

        // Remove table row
        $(document).on('click', '.remove-table-row', function() {
            $(this).closest('.table-input-card').remove();
        });
    });
</script>
@endsection