          ...
          <div class="card-body">
             <div class="row">
                ...
                <form action="{{ route('editPage') }}" enctype="multipart/form-data" method="post">
                    ...
                    <h3 class="mb-4 card-header"><i class="bi bi-file-earmark-break">Representative</i></h3>
                    <div class="form-group col-lg-8">
                        <label style="margin-top:15px">Representative Name</label>
                        <div class="input-group">
                            <input type="text" class="form-control" name="representative" value="{{ $page->representative ?? '' }}">
                        </div>
                    </div>

                    <h3 class="mb-4 card-header"><i class="bi bi-file-earmark-break">Professional Information</i></h3>
                    @if($errors->any())
                    <div class="alert alert-danger d-flex align-items-center" role="alert">
                        <svg class="bi flex-shrink-0 me-2" width="24" height="24">
                            <use xlink:href="#exclamation-triangle-fill"></use>
                    ...
                    <div></div>
                        <div class="form-group col-lg-8">
                            ...
                            <div class="input-group">
                                <input type="text" class="form-control" name="title" value="{{ $profinfo->title ?? '' }}">
                            </div>

                            <label style="margin-top:15px">Company / Organization</label>
                            <div class="input-group">
                                <input type="text" class="form-control" name="company" value="{{ $profinfo->company ?? '' }}">
                            </div>

                            <label style="margin-top:15px">Location</label>
                            <div class="input-group">
                                <input type="text" class="form-control" name="location" value="{{ $profinfo->location ?? '' }}">
                            </div>

                            <label style="margin-top:15px">Country</label>
                            <div class="input-group">
                                <input type="text" class="form-control" name="country" value="{{ $profinfo->country ?? '' }}">
                            </div>

                            <label style="margin-top:15px">E-Mail</label>
                            <div class="input-group">
                                <input type="email" class="form-control" name="email" value="{{ $profinfo->email ?? '' }}">
                            </div>

                            <label style="margin-top:15px">Phone Number</label>
                            <div class="input-group">
                                <input type="text" class="form-control" name="mobile" value="{{ $profinfo->mobile ?? '' }}">
                            </div>

                            <label style="margin-top:15px">Role</label>
                            <div class="input-group">
                                <input type="text" class="form-control" name="role" value="{{ $profinfo->role }}">
                            </div>
                      ...
                      <p class="text-muted">{{__('messages.disablesharebutton')}}</p>
                        ...
                        <div class="form-group col-lg-8">
                          ...
                          <p class="text-muted">{{__('messages.openlinksnewtab')}}</p>
                            ...
                            <div class="form-group col-lg-8">
                          <h5 style="margin-top:50px">{{__('messages.professionalabel')}}</h5>
                          <p class="text-muted">{{__('messages.hideprofessional')}}</p>
                            <div class="mb-3 form-check form-switch">
                              <input name="professional" class="switch toggle-btn" type="checkbox" id="professional" <?php if(UserData::getData(Auth::user()->id, 'show-professional') != false){echo 'checked';} ?> />
                              <label class="form-check-label" for="professional">{{__('messages.Enable')}}</label>
                            </div>

                            <div class="form-group col-lg-8">
                          <h5 style="margin-top:50px">{{__('messages.senddetailslabel')}}</h5>
                          <p class="text-muted">{{__('messages.hidesenddetails')}}</p>
                            <div class="mb-3 form-check form-switch">
                              <input name="details" class="switch toggle-btn" type="checkbox" id="details" <?php if(UserData::getData(Auth::user()->id, 'show-send-details') != false){echo 'checked';} ?> />
                              <label class="form-check-label" for="details">{{__('messages.Enable')}}</label>
                            </div>
                ...
                <script>
                  ClassicEditor
                      ...
                      document.getElementById('otherRolesCheckbox').addEventListener('change', function () {
                          const successLabel = document.getElementById('successLabel');
                          successLabel.style.display = this.checked ? 'block' : 'none';
                      });
